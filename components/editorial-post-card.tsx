...
export function EditorialPostCard({ post }: EditorialPostCardProps) {
  ...
  return (
    <article className="space-y-3.5 sm:space-y-5 lg:space-y-6">
      <Link href={`/post/${post.slug}`} className="block overflow-hidden rounded-2xl sm:rounded-3xl">
        <Image
          ...
          width={1200}
          height={720}
          className="h-44 w-full rounded-2xl object-cover sm:h-64 sm:rounded-3xl lg:h-72"
        />
      </Link>

      <div className="space-y-2.5 px-0.5 sm:space-y-4 sm:px-1">
        ...
        <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground sm:text-lg">{post.excerpt}</p>
