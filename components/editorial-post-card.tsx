import Image from "next/image"
import Link from "next/link"
import type { BlogPost } from "@/lib/types"
import { formatPublishedDate } from "@/lib/utils"

interface EditorialPostCardProps {
  post: BlogPost
}

export function EditorialPostCard({ post }: EditorialPostCardProps) {
  const coverImage = post.image || "/images/blog-cover-gradient.svg"

  return (
    <article className="space-y-5 md:space-y-6">
      <Link href={`/post/${post.slug}`} className="block overflow-hidden rounded-2xl">
        <Image
          src={coverImage}
          alt={post.title}
          width={1200}
          height={720}
          className="aspect-[16/10] w-full rounded-2xl object-cover"
        />
      </Link>

      <div className="space-y-3 px-1 md:space-y-4">
        <p className="text-sm font-medium text-muted-foreground/80">{formatPublishedDate(post.publishedAt)}</p>

        <Link href={`/post/${post.slug}`}>
          <h2 className="text-2xl font-semibold leading-tight tracking-tight text-foreground transition-colors hover:text-primary md:text-3xl">
            {post.title}
          </h2>
        </Link>

        <p className="max-w-3xl text-base leading-relaxed text-muted-foreground md:text-lg">{post.excerpt}</p>

        <p className="pt-1 text-xs uppercase tracking-[0.16em] text-muted-foreground/90">{post.category}</p>
      </div>
    </article>
  )
}
