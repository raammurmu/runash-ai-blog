...
import { SHARE_MESSAGES, copyTextToClipboard, createShareLinks, openSharePopup } from "@/lib/share"
...
export const ShareSidebar = ({ title }: { title: string }) => {
  ...
  const shareLinks = useMemo(() => createShareLinks({ title, url: pageUrl }), [title, pageUrl])
  ...
  const links = [
    { icon: <Twitter className="h-5 w-5" />, label: "Twitter", href: shareLinks.twitter },
    { icon: <Linkedin className="h-5 w-5" />, label: "LinkedIn", href: shareLinks.linkedin },
    { icon: <Facebook className="h-5 w-5" />, label: "Facebook", href: shareLinks.facebook },
  ]
